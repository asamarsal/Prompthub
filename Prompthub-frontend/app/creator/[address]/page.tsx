...
export default function CreatorProfilePage({ params }: { params: Promise<{ address: string }> }) {
  ...
  const mappedPrompts = prompts.map((p: any) => ({
    ...
    title: p.title,
    creator: p.user?.stx_address ?? creatorAddress,
    creatorName: p.user?.name ?? p.user?.username ?? "Creator",
    creatorAvatar: p.user?.avatar_url ?? "",
    price: parseFloat(p.price_sbtc ?? 0),
    currency: p.currency ?? "sBTC",
    rating: p.rating ?? 0,
    reviews: 0,
      ...
      <div className="mx-auto max-w-7xl px-4 py-10 lg:px-8">
        ...
        <div className="grid grid-cols-2 sm:grid-cols-4 gap-4 mb-10">
          {[
            ...
            { label: "Revenue (STX)", value: (stats.total_revenue ?? 0).toFixed(4), icon: TrendingUp, color: "text-[#b4ff39]", border: "border-[#b4ff39]/30" },
          ...
          <div className="flex items-center justify-between mb-3">
            <div className="flex items-center gap-2">
              <div className="w-8 h-8 bg-gradient-to-br from-[#ff2d95] to-[#00ffff] flex items-center justify-center text-xs font-extrabold text-white">
                {(review.user?.name ?? review.user?.stx_address ?? "AN").slice(0, 2).toUpperCase()}
              </div>
              <div>
                <span className="text-sm font-extrabold text-[#e0d4ff]">
                  {review.user?.name ?? review.user?.username ?? review.user?.stx_address?.slice(0, 8) ?? "Anonymous"}
                </span>

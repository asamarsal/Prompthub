...
import { getContest, getContestSubmissions, submitContestEntry, declareContestWinner, uploadFile } from "@/lib/api"
...
export default function ContestDetailPage({ params }: { params: Promise<{ id: string }> }) {
    ...
    const handleSubmitEntry = async () => {
        ...
        try {
            ...
            // We use a mock CID for now or a shortened hash
            ...
            await openContractCall({
                ...
                functionArgs: [
                    uintCV(1), // TODO: Use real on-chain ID if available, for now index 1
                    stringAsciiCV(mockCid)
                ],
                onFinish: async (data) => {
                    // 3. Sync to Backend
    ...
    const isOwner = myAddress === contest.brand_address
                    ...
                    <div className="md:col-span-1 flex flex-col gap-6">
                        ...
                        <div className="bg-[#0d0d0d] border-2 border-[#2a2a30] p-5" style={{ boxShadow: `5px 5px 0 0 ${accent}` }}>
                            ...
                            <div className="border-t border-[#2a2a30] pt-4 mb-4">
                                <p className="text-xs text-white/40 uppercase tracking-wider mb-3 flex items-center gap-1.5"><Trophy className="w-3.5 h-3.5 text-[#ff2d95]" /> Prize Pool: <span className="text-[#ff2d95] font-extrabold font-mono">{contest.total_prize_sbtc} sBTC</span></p>
                                <div className="flex flex-col gap-1.5">
                                    {(contest.prize_tiers || []).map((p: any, i: number) => (
                                        <div key={i} className="flex justify-between text-xs">
                                            ...
                                            <span className="font-mono font-bold text-[#ff2d95]">{p.prize_sbtc} sBTC</span>
                                        </div>
                                    ))}
                                </div>
                            </div>

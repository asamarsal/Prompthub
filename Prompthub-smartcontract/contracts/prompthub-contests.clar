...
(define-map contests
  ...
  {
    brand: principal,
    total-pool: uint,        ;; Total STX escrowed (sum of all tiers)
...
;; =====================
(define-public (declare-winner (contest-id uint) (place uint) (winner principal))
  (let
    (
      (contest (unwrap! (map-get? contests contest-id) err-contest-not-found))
      (tier (unwrap! (map-get? prize-tiers { contest-id: contest-id, place: place }) err-tier-not-found))
...
;; =====================
(define-public (cancel-contest (contest-id uint))
  (let
    (
      (contest (unwrap! (map-get? contests contest-id) err-contest-not-found))
      (brand (get brand contest))
...
;; =====================
(define-public (declare-winner-external (contest-id uint) (place uint) (winner principal))
  (let
    (
      (contest (unwrap! (map-get? contests contest-id) err-contest-not-found))
      (tier (unwrap! (map-get? prize-tiers { contest-id: contest-id, place: place }) err-tier-not-found))

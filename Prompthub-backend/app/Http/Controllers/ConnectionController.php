...
class ConnectionController extends Controller
{
    ...
    {
        $address = $request->user()->stx_address;

        $connections = Connection::where('requester_address', $address)
            ->orWhere('recipient_address', $address)
            ->with(['requester:stx_address,name,avatar_url', 'recipient:stx_address,name,avatar_url'])
            ->get();
    ...
    {
        $request->validate([
            'recipient_address' => 'required|string|exists:users,stx_address'
        ]);

        $requester_address = $request->user()->stx_address;
    ...
    public function accept(Request $request, $id)
    {
        ...
        if ($connection->recipient_address !== $request->user()->stx_address) {

...
const wallets = [
  { id: "leather", name: "Leather Wallet", recommended: true },
  { id: "xverse", name: "Xverse Wallet", recommended: false },
  { id: "walletconnect", name: "WalletConnect", recommended: false },
]
...
export function ConnectWalletModal({ open, onClose }: { open: boolean; onClose: () => void }) {
  ...
  return (
    <Dialog open={open} onOpenChange={(o) => !o && onClose()}>
      <DialogContent className="bg-[#0a001a]/95 border border-[rgba(180,120,255,0.25)] backdrop-blur-xl max-w-md text-[#e0d4ff] shadow-[0_0_60px_rgba(255,45,149,0.1)]">
        <DialogHeader>
        ...
        {isConnected ? (
          <div className="flex flex-col gap-4 py-2">
            ...
            <div className="flex items-center justify-between p-4 rounded-xl glass">
              ...
              <span className="text-lg font-extrabold text-[#00ffff]">{balance.toFixed(4)} sBTC</span>
            </div>
          ...
          <div className="flex flex-col gap-3 py-2">
            <p className="text-sm text-[#a78bfa]">
              Choose a wallet to connect to PromptChain
            </p>
            {wallets.map((w) => (
              <button
                key={w.id}
